  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-earth-200 shadow-sm">
        <div>
          <h1 className="text-2xl font-black text-slate-900">{t('myCrop')}</h1>
          <p className="text-sm text-slate-500 mt-1">
            {language === 'en' 
              ? 'List and manage crop harvests ready for sale.' 
              : 'విక్రయించడానికి సిద్ధంగా ఉన్న మీ పంటలను నమోదు చేసి నిర్వహించండి.'}
          </p>
        </div>

        <Link
          to="/produce/new"
          className="inline-flex items-center px-5 py-2.5 rounded-xl bg-agri-800 text-white text-xs font-bold hover:bg-agri-700 shadow-sm transition"
        >
      ...
      {loading ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-earth-200">
          ...
          <p className="mt-3 text-sm text-slate-500 font-semibold">Loading harvests...</p>
        </div>
      ) : listings.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-earth-200 p-8 space-y-4">
          ...
          <h3 className="text-lg font-bold text-slate-900">
            {language === 'en' ? 'No Harvest Listed Yet' : 'ఇంకా ఎటువంటి పంట నమోదు కాలేదు'}
          </h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            {language === 'en' 
              ? 'Add your crop quantity and location to calculate options.' 
              : 'అమ్మకపు ఎంపికలను లెక్కించడానికి మీ పంట పరిమాణం మరియు జిల్లా వివరాలను జోడించండి.'}
          </p>
          <Link
            to="/produce/new"
            className="inline-flex items-center px-6 py-3 rounded-xl bg-agri-800 text-white text-xs font-bold hover:bg-agri-700 shadow-sm transition"
          >
            <Plus className="w-4 h-4 mr-1.5" />
            {t('sellMyCrop')}
          </Link>
            ...
            >
              <div>
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <h3 className="text-base font-extrabold text-slate-900 mt-1">{item.varietyName || 'FAQ'}</h3>
                  </div>
                  <span className="text-[10px] font-extrabold px-2 py-0.5 rounded bg-slate-100 text-slate-700 border">
                ...
                <div className="space-y-2 text-xs text-slate-600">
                  <div className="flex items-center justify-between py-1 border-b border-earth-100">
                    <span className="text-slate-500">{t('quantityLabel')}:</span>
                    <strong className="text-slate-900 font-extrabold">{item.quantityKg} kg</strong>
                  </div>
                  ...
                  <div className="flex items-center justify-between py-1 border-b border-earth-100">
                    ...
                    <span className="font-semibold text-slate-800">{item.locationDistrict}, {item.locationState}</span>
                  </div>
              ...
              <div className="pt-3 border-t border-earth-100">
                <Link
                  ...
                  className="w-full py-2.5 px-4 rounded-xl bg-agri-800 text-white text-xs font-bold hover:bg-agri-700 flex items-center justify-center space-x-2 transition shadow-sm"
                >
